...
# (the path through the straight corridor) is baked offline in Mock by
...
# ros_gazebo_bridge/worlds/straight_deploy.*):
#     python scripts/export_mock_trajectory.py --zigzag-amp 0 \
#         --sacr-ckpt checkpoints/mock/sacr.pt --camr-ckpt checkpoints/mock/camr.pt \
#         --policy-ckpt checkpoints/mock/ppo.pt --out renders/deploy/straight
#
# It:
#   1. brings up the sim with the MATCHED world (PX4_GZ_WORLD=straight_deploy.sdf),
...
INFERENCE="${INFERENCE:-renders/deploy/straight_inference.csv}"
...
export PX4_GZ_WORLD=/worlds/straight_deploy.sdf
...
SRC_PREFIX="${SRC_PREFIX:-renders/deploy/straight}"   # tracked canonical export
...
# PX4's gz-bridge derives the gz world NAME from the .sdf filename (-w straight_deploy),
...
sed 's/<world name="[^"]*">/<world name="straight_deploy">/' "${SRC_PREFIX}.sdf" > ros_gazebo_bridge/worlds/straight_deploy.sdf
cp "${SRC_PREFIX}.world.json" ros_gazebo_bridge/worlds/straight_deploy.world.json

say "1/5  Bringing up the sim with the MATCHED straight world ..."
...
if grep -q "<name>worker" ros_gazebo_bridge/worlds/straight_deploy.sdf; then
...
docker cp "$REPO/ros_gazebo_bridge/worlds/straight_deploy.world.json" "$ROSC:$WORLDS_PKG/straight_deploy.world.json" 2>/dev/null

say "4/5  Bridging ground-truth odometry + worker cmd_vel ..."
NW=$(grep -c "<name>worker" ros_gazebo_bridge/worlds/straight_deploy.sdf || echo 0)
WORKER_BRIDGES=""
...
say "done. (path <- Mock best straight-corridor policy; world <- matched straight_deploy.sdf)"
